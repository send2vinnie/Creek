...
namespace Vestris.ResourceLib
{
    ...
    public class ResourceInfo : IEnumerable<Resource>, IDisposable
    {
        ...
        private static bool _duringBatchUpdate = false;
        private static IntPtr _hBatchUpdate = IntPtr.Zero;
        private static string _fileBatchUpdate = null;

        internal static bool duringBatchUpdate
        {
            get
            {
                return _duringBatchUpdate;
            }
        }

        internal static IntPtr handlerBatchUpdate
        {
            get
            {
                return _hBatchUpdate;
            }
        }

        internal static string fileBatchUpdate
        {
            get
            {
                return _fileBatchUpdate;
            }
        }

        internal static void BeginBatchUpdate(string filename)
        {
            _hBatchUpdate = Kernel32.BeginUpdateResource(filename, false);
            if (_hBatchUpdate == IntPtr.Zero)
                throw new Win32Exception(Marshal.GetLastWin32Error());
            _fileBatchUpdate = filename;
            _duringBatchUpdate = true;
        }

        internal static void EndBatchUpdate()
        {
            if (!Kernel32.EndUpdateResource(_hBatchUpdate, false))
                throw new Win32Exception(Marshal.GetLastWin32Error());
            _hBatchUpdate = IntPtr.Zero;
            _fileBatchUpdate = "";
            _duringBatchUpdate = false;
        }
